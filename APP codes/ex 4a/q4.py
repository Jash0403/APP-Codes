from tkinter import *
master = Tk()
v = IntVar()
options = []
for i in range(1, 16):
    options.append(i)
Label(master, text="Bookingid").grid(row=0)
Label(master, text="Passenger Name").grid(row=1)
Label(master, text="Flight").grid(row=2)
Label(master, text="Source").grid(row=3)
Label(master, text="Destination").grid(row=3, column=3)
Label(master, text="Duration").grid(row=4)
menu = StringVar()
menu.set("Select Flight Duration")
# entry fields
e1 = Entry(master)
e2 = Entry(master)
e3 = Entry(master)
r1 = Radiobutton(master, text="Delhi", variable=v, value=1)
r2 = Radiobutton(master, text="Mumbai", variable=v, value=2)
r3 = Radiobutton(master, text="Chennai", variable=v, value=3)
r4 = Radiobutton(master, text="Kolkata", variable=v, value=4)
drop = OptionMenu(master, menu, *options)
b1 = Button(master, text="Insert")
b2 = Button(master, text="Update")
b3 = Button(master, text="Delete")
b4 = Button(master, text="Select")
# Place fields
e1.grid(row=0, column=1)
e2.grid(row=1, column=1)
e3.grid(row=2, column=1)
r1.grid(row=3, column=1)
r2.grid(row=3, column=2)
r3.grid(row=3, column=4)
r4.grid(row=3, column=5)
drop.grid(row=4, column=1)
b1.grid(row=5, column=0)
b2.grid(row=5, column=1)
b3.grid(row=6, column=0)
b4.grid(row=6, column=1)
mainloop()
