...
window.geometry(
    "{0}x{0}+0+0".format(window.winfo_screenwidth(), window.winfo_screenheight()))
Label(window, text="Job Application", font=('Helvetica 24 bold'),
      fg="black").place(relx=0.5, rely=0.03, anchor=CENTER)
Label(window, text="Personal Information", font="Helvetica 16 bold",
      fg="red").grid(row=1, column=1, pady=(40, 0))
...
education_chosen = ttk.Combobox(
    window, width=37, textvariable=dropdown1, state="readonly")
...
def UploadAction(event=None):
    path = filedialog.askopenfilename(
        filetypes=[('PDF Files', '.pdf'), ('DOC Files', '.docx')])
    if path:
        filename = os.path.basename(path)
        Label(window, text="Selected File is: ",
              font="Helvetica 12", fg="red").grid(row=5, column=3)
        Label(window, text=filename, font="Helvetica 12",
              fg="black").place(x=850, y=215)
    else:
        Label(window, text="File Not Selected!",
              font="Helvetica 14", fg="red").grid(row=5, column=3)


button = tk.Button(window, text='Upload File', width=34,
                   command=UploadAction).grid(row=5, column=2)
...
country_chosen = ttk.Combobox(
    window, width=35, textvariable=dropdown2, state="readonly")
...
Label(window, text="What are your hobbies?",
      font="Helvetica 14").grid(row=11, column=1)
text11 = Entry(window, width=40).grid(row=11, column=2)

Label(window, text="Precious/Current Employment Details",
      font="Helvetica 16 bold", fg="red").grid(row=12, column=1, pady=(20, 0))
...
Label(window, text="How long were you here?: ",
      font="Helvetica 14").grid(row=15, column=1)
text14 = Entry(window, width=40).grid(row=15, column=2)


Label(window, text="Reference #1", font="Helvetica 14 bold",
      fg="red").grid(row=16, column=1, pady=(10, 0))
...
Label(window, text="Reference #2", font="Helvetica 14 bold",
      fg="red").grid(row=19, column=1, pady=(10, 0))
...
button_1 = Button(window, text="SUBMIT", padx=10, pady=5, bg="black",
                  fg="white").place(relx=0.5, rely=0.89, anchor=CENTER)
